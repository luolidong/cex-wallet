...
func TestConfirmOnceReceiptNotFoundStaysPending(t *testing.T) {
	...
	defer rpc.Close()
	apiServer, confirmed, failed := newAPIServer(t, rpc.URL, 2)
	...
	if result.Pending != 1 || result.Confirmed != 0 || result.Failed != 0 || confirmed.Load() != 0 || failed.Load() != 0 {
		t.Fatalf("unexpected result: %+v confirmedCalls=%d failedCalls=%d", result, confirmed.Load(), failed.Load())
	}
}

func TestConfirmOnceRevertedReceiptFailsWithdrawal(t *testing.T) {
	...
	if result.Failed != 1 || result.Confirmed != 0 || confirmed.Load() != 0 || failed.Load() != 1 {
		t.Fatalf("unexpected result: %+v confirmedCalls=%d failedCalls=%d", result, confirmed.Load(), failed.Load())
	}
}
...
func TestConfirmOnceSuccessfulReceiptConfirmsAfterThreshold(t *testing.T) {
	...
	if result.Confirmed != 1 || result.Pending != 0 || result.Failed != 0 || confirmed.Load() != 1 || failed.Load() != 0 {
		t.Fatalf("unexpected result: %+v confirmedCalls=%d failedCalls=%d", result, confirmed.Load(), failed.Load())
	}
}
...
func newAPIServer(t *testing.T, rpcURL string, confirmBlocks int) (*httptest.Server, *atomic.Int32, *atomic.Int32) {
	t.Helper()
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case r.Method == http.MethodPost && r.URL.Path == "/api/internal/scanner/withdrawals/failed":
			failed.Add(1)
			var body api.FailWithdrawalRequest
			if err := json.NewDecoder(r.Body).Decode(&body); err != nil {
				t.Fatalf("decode fail request: %v", err)
			}
			if body.WithdrawalID != 1 || body.TxHash != "0xtx" || body.Reason == "" {
				t.Fatalf("unexpected fail request: %+v", body)
			}
			_ = json.NewEncoder(w).Encode(map[string]any{
				"success": true, "message": "ok",
				"data": map[string]any{"id": 1, "status": "FAILED", "txHash": "0xtx"},
			})
		default:
			http.NotFound(w, r)
		}
	}))
	return server, confirmed, failed
}
